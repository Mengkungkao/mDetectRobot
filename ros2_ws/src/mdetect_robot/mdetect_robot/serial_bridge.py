...
class SerialBridge(Node):
    def __init__(self) -> None:
        ...
        self.wheel_speed_pub = self.create_publisher(Float32MultiArray, '/base/wheel_speeds_mm_s', 20)
        self.wheel_pwm_pub = self.create_publisher(Float32MultiArray, '/base/wheel_pwm', 20)
        self.encoder_pub = self.create_publisher(Int32MultiArray, '/base/encoder_ticks', 20)
        self.serial_rx_pub = self.create_publisher(String, '/base/serial_rx', 20)
        self.tf_broadcaster = TransformBroadcaster(self)

        self.create_subscription(Twist, cmd_topic, self.cmd_vel_callback, 20)
        self.create_subscription(String, '/base/config_command', self.config_command_callback, 10)
    ...
    def config_command_callback(self, msg: String) -> None:
        command = msg.data.strip().upper()
        allowed_prefixes = ('PID,', 'PIDM,', 'TRIM,', 'TRIMF,', 'TRIMR,', 'HEADING,', 'GET_CONFIG', 'PING')
        if not command.startswith(allowed_prefixes):
            self.get_logger().warning(f'Rejected unsafe config command: {command!r}')
            return
        if not self.send_line(command):
            self.get_logger().error('Could not send configuration command to Arduino')
    ...
    def handle_serial_line(self, line: str) -> None:
        ...
        serial_message.data = line
        self.serial_rx_pub.publish(serial_message)
    ...
    def publish_telemetry(self, data: Telemetry) -> None:
        ...
        wheel_speed = Float32MultiArray()
        wheel_speed.data = [float(value) for value in data.wheel_speed_mm_s]
        self.wheel_speed_pub.publish(wheel_speed)

        wheel_pwm = Float32MultiArray()
        wheel_pwm.data = [float(value) for value in data.wheel_pwm]
        self.wheel_pwm_pub.publish(wheel_pwm)

        encoder_ticks = Int32MultiArray()
        encoder_ticks.data = [int(value) for value in data.ticks]
        self.encoder_pub.publish(encoder_ticks)
