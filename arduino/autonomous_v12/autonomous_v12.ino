/*
  ...
  Test aliases:
    ...
    HEADING,<kp>,<max_correction_deg_s>
    GET_CONFIG
    PING
...
// reverse. Motor 4 is faster in both directions and especially in reverse.
float PWM_FORWARD_SCALE[MOTOR_COUNT] = {1.0f, 1.0f, 1.0f, 0.82f};
float PWM_REVERSE_SCALE[MOTOR_COUNT] = {1.0f, 1.0f, 1.0f, 0.66f};

// Straight-line heading hold uses the MPU6050 to remove residual drift caused
// by tyre loading, floor friction, and small encoder calibration errors.
float HEADING_HOLD_KP = 2.0f;
float HEADING_HOLD_MAX_DEG_S = 22.0f;
const float HEADING_HOLD_LINEAR_MIN_MM_S = 35.0f;
const float HEADING_HOLD_ANGULAR_DEADBAND_DEG_S = 1.0f;
...
void setVelocityCommand(float linearMMs, float angularDegS) {
  ...
  const bool straightCommand =
    fabs(newLinear) >= HEADING_HOLD_LINEAR_MIN_MM_S &&
    fabs(newAngular) <= HEADING_HOLD_ANGULAR_DEADBAND_DEG_S;
  ...
  if (straightCommand && !headingHoldActive) {
    headingTargetDeg = yawDeg;
    headingHoldActive = true;
  } else if (!straightCommand) {
    headingHoldActive = false;
  }
...
void processCommand(char *line) {
  ...
  if (strcmp(command, "VEL") == 0) {
    ...
    if (kpText && maxText) {
      HEADING_HOLD_KP = constrain(atof(kpText), 0.0f, 10.0f);
      HEADING_HOLD_MAX_DEG_S = constrain(atof(maxText), 0.0f, 60.0f);
      Serial.println(F("ACK,HEADING"));
    } else {
      Serial.println(F("ERR,HEADING_FORMAT"));
    }
  }
  else if (strcmp(command, "GET_CONFIG") == 0) {
    Serial.print(F("CONFIG,TRIMF"));
    for (uint8_t i = 0; i < MOTOR_COUNT; ++i) { Serial.print(','); Serial.print(PWM_FORWARD_SCALE[i], 3); }
    Serial.print(F(",TRIMR"));
    for (uint8_t i = 0; i < MOTOR_COUNT; ++i) { Serial.print(','); Serial.print(PWM_REVERSE_SCALE[i], 3); }
    Serial.print(F(",HEADING,")); Serial.print(HEADING_HOLD_KP, 3);
    Serial.print(','); Serial.println(HEADING_HOLD_MAX_DEG_S, 3);
  }
...
void updateControl(float dt) {
  ...
  float correctedAngularDegS = rampedAngularDegS;
  if (headingHoldActive &&
      fabs(rampedLinearMMs) >= HEADING_HOLD_LINEAR_MIN_MM_S &&
      fabs(commandAngularDegS) <= HEADING_HOLD_ANGULAR_DEADBAND_DEG_S) {
    const float headingErrorDeg = normalizeDegrees(headingTargetDeg - yawDeg);
    const float correctionDegS = constrain(
      HEADING_HOLD_KP * headingErrorDeg,
      -HEADING_HOLD_MAX_DEG_S,
      HEADING_HOLD_MAX_DEG_S
    );
    correctedAngularDegS += correctionDegS;
  }

  const float angularRadS = correctedAngularDegS * DEG_TO_RAD;
