...
bool Lidar_Data_Processing::wait_start_reply(uint64_t timeout)
{
  ...
  while ((waitTime = current_times() - startTs) <= timeout){
      size_t remainSize = 12;
      size_t recvSize = 0;
      result_t ans = node_lidar.serial_port->waitForData(remainSize, timeout - waitTime, &recvSize);
      if (!IS_OK(ans)) {
        printf("lidar_start_reply waitForData fail---\n");
        return false;
      }
      ...
      if (IS_FAIL(ans)) {
        printf("lidar_start_reply read_data fail---\n");
        return false;
      }
      ...
      if(checkvalue == checkdata && recvPos==12){
        printf("lidar_start_reply--------OK,%x,%x\n",checkvalue,checkdata);
        return true;
        break;
      }
    }
    printf("lidar_start_reply-----------error\n");
    return false;
}
